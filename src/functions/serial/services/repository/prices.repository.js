...
const bulkAddManyProducts = async (products) => {
  ...
  await Promise.all(
    productsToCreate.map(async (productToCreate) => {
      ...
      const productBaseSqlToInsert = pricesMapper.formatObjToInsertSql(productBase, "TCHAPR018120");
      ...
      const responseOutputProductBase = await repository.executeInsert(productBaseSqlToInsert, []);
      ...
      feedbackProductsCreate.push({ productBase, responseOutputProductBase, /*responseOutputProductDesciptions, responseOutputProductDistributor, responseOutputProductSendData */ });
      return null;
    })
  );

  return {
    Message: "hello world bulkAddManyProducts",
    productsToCreate,
    productsInStock,
    feedbackProductsCreate,
  };
};
