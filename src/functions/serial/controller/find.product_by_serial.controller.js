...
const findProductBySerial = async (req, res) => {
  const requestErrors = await requestValidator.validateRequest(productBySerialSchemaValidations, req);
  if (requestErrors.length > 0) {
    return responseUtil.buildErrorResponse(res, requestErrors);
  }
  const prices = await pricesRepository.findProductBySerial(req.query.SerialCode);
